...
resource "cloudstack_network" "bosh_subnet" {
  ...
  zone = "${var.cloudstack_zone}"
  acl_id = "${var.secure ? element(concat(cloudstack_network_acl.bosh_subnet_sec_group.*.id, list("")), 0) : cloudstack_network_acl.allow_all.id}"
}
...
resource "cloudstack_network" "control_plane" {
  ...
  network_offering = "${var.network_vpc_offering}"
  zone = "${var.cloudstack_zone}"
  acl_id = "${var.secure ? element(concat(cloudstack_network_acl.control_plane_sec_group.*.id, list("")), 0) : cloudstack_network_acl.allow_all.id}"
}
...
resource "cloudstack_network" "data_plane" {
  ...
  network_offering = "${var.network_vpc_offering}"
  zone = "${var.cloudstack_zone}"
  acl_id = "${var.secure ? element(concat(cloudstack_network_acl.data_plane_sec_group.*.id, list("")), 0) : cloudstack_network_acl.allow_all.id}"
}
...
output "internal_subnet_cidr_mapping" {
  value = "${
    map(
      ...
      "${element(concat(cloudstack_network.data_plane_public.*.name, list("")), 0)}", "${element(concat(cloudstack_network.data_plane_public.*.cidr, list("")), 0)}",
...
output "internal_subnet_gw_mapping" {
  value = "${
    map(
      ...
      "${element(concat(cloudstack_network.data_plane_public.*.name, list("")), 0)}", "${element(concat(cloudstack_network.data_plane_public.*.gateway, list("")), 0)}",
